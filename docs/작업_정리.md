...
npm run dev
```
- "단일 이미지 모드"면 기존 단일 이미지 + 오버레이. "WSI" 드롭다운에서 슬라이드를 선택하면 타일 서버에서 해당 슬라이드를 불러와 OpenSeaDragon으로 표시.

**타일 서버 (WSI 뷰어 사용 시)**
- Python 3.10+, OpenSlide C 라이브러리 설치. `tile-server/README.md` 참고.
- `WSI_DIR` 환경 변수로 WSI 파일 디렉터리 지정 후 `uvicorn app.main:app --reload --port 8000`.
- 프론트에서 타일 서버 URL이 다르면 `.env`에 `VITE_TILE_SERVER_URL` 설정.

---

##### 12. 타일 서버 및 WSI 뷰어

- **타일 서버** (`tile-server/`): FastAPI + OpenSlide. 원본 WSI를 on-demand로 타일(256×256) 생성해 반환. API: `GET /api/slides`, `GET /api/slides/{id}/info`, `GET /api/slides/{id}/tiles/{level}/{x}_{y}.jpg`.
- **프론트:** 슬라이드 목록은 React Query `['slides']`, 선택 슬라이드 메타정보는 `['slideInfo', id]`. OpenSeaDragon으로 타일 표시, 뷰포트 변경 시 deck.gl viewState를 동기화해 마커·tumor area 오버레이가 슬라이드와 맞춰지도록 함.
- **상세 구현·학습 노트:** [docs/구현_로그.md](docs/구현_로그.md).
...
- 타일 서버: 디스크 캐시, IIIF/DZI 표준 호환, 인증 등 확장 가능.
- 계획 상세: `.cursor/plans/이미지_마커_오버레이_poc.plan.md`, `.cursor/plans/wsi_타일_서버_및_osd_연동_*.plan.md`.
